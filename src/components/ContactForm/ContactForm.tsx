...
export default function ContactForm() {
          ...
          {status === FormStatus.SENT ? (
            <>
              ...
              <div className={styles.feedback__text}>
                {pageContent.contact.success.map((msg, idx) => (
                  <span key={idx}>{msg}</span>
                ))}
              </div>
              <div className={styles.feedback__image}>
                <Image src={ThumbsUp} alt="Sucesso no contato" />
              </div>
            </>
          ) : (
            <>
              ...
              />
              <div className={styles.feedback__text}>
                {pageContent.contact.failure.map((msg, idx) => (
                  <span key={idx}>{msg}</span>
                ))}
              </div>
              <div className={styles.feedback__image}>
                <Image src={ThumbsDown} alt="Falha no contato" />
              </div>
            </>
          )}
        </Container>
  ...
  return !formOpen ? (
    <button
      className={`${styles.fixedContainer} ${styles.contactButton}`}
      onClick={handleOpenForm}
    >
      <Image src={Email} alt="Entre em contato" />
    </button>
    ...
    >
      ...
      {formStatus === FormStatus.READY ? (
        <input
          className={styles.contactForm__send}
          type="submit"
          value={pageContent["contact"]["send"]}
        />
      ) : (
